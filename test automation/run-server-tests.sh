...
# TalentHive Server Test Runner with Error Filtering
# Runs all tests, creates detailed logs, and shows only failures

set -e

echo "=========================================="
echo "TALENTHIVE SERVER - TEST RUNNER"
echo "=========================================="
echo ""
...
# Run tests
echo -e "${YELLOW}Running all server tests...${NC}"
echo ""

cd "$SERVER_DIR"

# Run tests and capture output
if npm test 2>&1 | tee "$MAIN_LOG"; then
    TEST_EXIT_CODE=0
else
    TEST_EXIT_CODE=$?
fi
...
TOTAL_SUITES=$(grep -o "Test Suites.*" "$MAIN_LOG" | tail -1 || echo "")
...
if grep -q "failed" "$MAIN_LOG"; then
    ...
    grep -B 2 -A 10 "FAIL\|Error:\|Expected\|Received\|●" "$MAIN_LOG" | \
        grep -v "node_modules\|at Object\|at process\|at Promise" | \
        head -100 >> "$ERROR_LOG" 2>/dev/null || true
    
    # Show failed test names
    echo -e "${RED}Failed Tests:${NC}" | tee -a "$SUMMARY_LOG"
    grep "FAIL.*test\\.ts" "$MAIN_LOG" | sed 's/.*FAIL  /  ✗ /' | tee -a "$SUMMARY_LOG"
    echo "" | tee -a "$SUMMARY_LOG"
...
echo "  Latest log:    server/test-logs/latest.log"
echo "  Latest errors: server/test-logs/latest-errors.log"
echo "  Latest summary: server/test-logs/latest-summary.txt"
echo ""
